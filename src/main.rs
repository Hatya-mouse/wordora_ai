...
impl MarkovChain {
    ...
    fn learn(&mut self, text: &str) {
        ...
        for word in whitespace_separated {
            let tokens = separate_japanese_tokens(word);
            japanese_separated.extend(tokens);
        }

        for word in japanese_separated {
            separated.extend(chunk_string(&word, 3));
        }
...
fn chunk_string(input: &str, chunk_size: usize) -> Vec<String> {
    input
        ...
        .collect() // Vec<String>にまとめる
}

// **📌 日本語を「漢字」「ひらがな」「カタカナ」「記号」ごとに分割**
fn separate_japanese_tokens(text: &str) -> Vec<String> {
...
そうだね、少しでも外に出て、自然を感じる時間を作ってみようと思うよ。今日もいい天気だから、何か予定を立てたくなってきた！
それはいいアイデアだね！今日は天気がいいから、気持ちよく外で過ごせそうだよ。何か楽しいことをして、リフレッシュできるといいね。";
    chain.learn(text);
    ...
    loop {
        ...
        let mut tokens = separate_japanese_tokens(input);
        tokens = chunk_string(tokens.join("").as_str(), 3);
        let start_word = tokens
            .first()
            .cloned()
            .unwrap_or_else(|| "こんにちは".to_string());
